...
- [x] 5.2 Launch switch in `bruchner_dev_response_headers` (`deploy/README.md`): CSP from report-only to enforced, remove `noindex`. Live 2026-09-25: enforced CSP, no `X-Robots-Tag`; verify `robots.txt`, `sitemap-index.xml`, `feed.xml` all emit `bruchner.dev`
- [x] 5.3 TTL: nothing to lower, phhbr.de was already at deSEC's minimum of 3600 s
- [x] 5.4 (2026-09-25; every legacy URL checked end to end: 1–2 permanent hops to a 200 on bruchner.dev, Let's Encrypt certs for phhbr.de and www) Add the phhbr.de block to the edge Caddy; repoint phhbr.de **A/AAAA only** from the GitHub Pages IPs to the VPS (MX, SPF, DKIM, DMARC, Proton verification untouched). Confirm cert issuance and path-preserving 301s
